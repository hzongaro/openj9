...
TR_J9ByteCodeIlGenerator::genDLTransfer(TR::Block *firstBlock)
   ...
   if (fej9()->isLiveMonitorMapCorrectnessRequired() && _methodSymbol->mayContainMonitors())
      ...
      TR_BitVector hasMonents(numBlocks, trMemory(), stackAlloc);
      TR_BitVector visited(numBlocks, trMemory(), stackAlloc);

      // Record blocks that contain monent operations in hasMonents BitVectore
      //
      for (TR::Block *block = firstBlock; block; block = block->getNextBlock())
         ...
         for (; tt != block->getExit(); tt = tt->getNextTreeTop())
            ...
            if (node->getOpCodeValue() == TR::monent
               ...
               {
               hasMonents.set(block->getNumber());
               break;
               }
            }
         }

      // Check whether any path leading back to the first block from blocks
      ...
      for (int32_t targetIdx = 0; targetIdx < haveSeenCount; targetIdx++)
         {
         if (hasMonentOnPathFromEntry(haveSeenTargets[targetIdx]->getNode()->getBlock(), hasMonents, visited))
            {
...
bool
TR_J9ByteCodeIlGenerator::hasMonentOnPathFromEntry(TR::CFGNode *currBlock, TR_BitVector &hasMonent, TR_BitVector &visited)
   {
   ...
   for (TR::CFGEdge *currEdge = pit.getFirst(); currEdge != NULL; currEdge = pit.getNext())
      ...
      if (hasMonent.isSet(predBlockIdx))
         ...
         if (hasMonentOnPathFromEntry(predBlock, hasMonent, visited))
            {
            return true;
            }
         }
