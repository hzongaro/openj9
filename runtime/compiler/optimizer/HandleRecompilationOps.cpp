...
TR_HandleRecompilationOps::visitNode(TR::TreeTop *currTree, TR::Node *node)
   ...
   if (opcode.isResolveCheck() && resolveCHKGuardsValueTypeOperation(currTree, node))
      ...
      if (_enableTransform
         ...
         // ???? Why do I need this edge?  The catch block is a successor of the induce block, not the current block
         ...
         comp()->failCompilation<TR::UnsupportedValueTypeOperation>("TR_HandleRecompilationOps encountered ResolveCHK in node n%dn [%p] guarding value type operation, but transformation is not enabled", node->getGlobalIndex(), node);
         }
      }
   }
