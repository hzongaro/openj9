...
void ForceHelperTransform::visitNode(TR::Node * node, TR::NodeChecklist &visited)
   ...
   if (node->getOpCode().isCall())
      ...
      const bool isNonNullableCheck =
            (node->getSymbolReference() ==
                comp()->getSymRefTab()->findOrCreateNonNullableArrayNullStoreCheckSymbolRef());

      if (isNonNullableCheck)
         {
         if (performTransformation(
               comp(),
               "%sChanging n%un from <nonNullableArrayNullStoreCheck> to PassThrough\n",
               OPT_DETAILS,
               node->getGlobalIndex()))
            {
            // Replace the <nonNullableArrayNullStoreCheck> with a PassThrough
            TR::Node *firstChild = node->getFirstChild();
            TR::Node *secondChild = node->getSecondChild();
            TR::Node::recreateWithoutProperties(node, TR::PassThrough, 1, firstChild);
