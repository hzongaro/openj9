...
AcmpTransformer::lower(TR::Node * const node, TR::TreeTop * const tt)
   ...
   if (!anchoredCallArg1TT->getNode()->getFirstChild()->isNonNull())
      ...
      const char *firstNullTestCounterName = TR::DebugCounter::debugCounterName(comp, "vt-helper/inline-check/LHS-is-null/acmp/(%s)/bc=%d",
                                                                  comp->signature(), checkLhsNull->getByteCodeIndex());
   ...
   if (!anchoredCallArg2TT->getNode()->getFirstChild()->isNonNull())
      ...
      const char *secondNullTestCounterName = TR::DebugCounter::debugCounterName(comp, "vt-helper/inline-check/RHS-is-null/acmp/(%s)/bc=%d",
                                                                  comp->signature(), checkRhsNull->getByteCodeIndex());
...
NonNullableArrayNullStoreCheckTransformer::lower(TR::Node* const node, TR::TreeTop* const tt)
   ...
   if (!sourceChild->isNonNull())
      ...
      static char *enableCheckNullFirstInNullStoreCheck = feGetEnv("TR_EnableCheckNullFirstInNullStoreCheck");
      if (enableCheckNullFirstInNullStoreCheck)
         {
         ...
         prevBlock = prevBlock->split(ifArrayCompClassValueTypeTT, cfg);
         prevBlock->setIsExtensionOfPreviousBlock(true);

         if (enableTrace)
             traceMsg(comp(),"ifArrayCompClassValueTypeTT n%dn is isolated in prevBlock %d\n", ifArrayCompClassValueTypeTT->getNode()->getGlobalIndex(), prevBlock->getNumber());
         }
