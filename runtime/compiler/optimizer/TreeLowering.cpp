...
TR::TreeLowering::lowerLoadArrayElement(TR::Node *node, TR::TreeTop *tt)
   ...
   TR::ILOpCodes opToCheck = TR::BadILOp;
...
#if 1
   if (opToCheck == TR::aRegStore)
      {
      TR_ASSERT_FATAL(anchoredNode->getOpCodeValue() == TR::aRegLoad && storeNode->getOpCodeValue() == TR::aRegStore && storeArrayElementNode->getOpCodeValue() == aRegStore,
      "op TR::aRegStore: anchoredNode n%dn %s, storeNode n%dn %s, storeArrayElementNode n%dn %s\n",
         anchoredNode->getGlobalIndex(), anchoredNode->getOpCode().getName(),
         storeNode->getGlobalIndex(), storeNode->getOpCode().getName(),
         storeArrayElementNode->getGlobalIndex(), storeArrayElementNode->getOpCode().getName());
      }
   else if (opToCheck == TR::astore)
      {
      TR_ASSERT_FATAL(anchoredNode->getOpCodeValue() == TR::aload && storeNode->getOpCodeValue() == TR::astore && storeArrayElementNode->getOpCodeValue() == TR::astore,
      "op TR::astore: anchoredNode n%dn %s, storeNode n%dn %s, storeArrayElementNode n%dn %s\n",
         anchoredNode->getGlobalIndex(), anchoredNode->getOpCode().getName(),
         storeNode->getGlobalIndex(), storeNode->getOpCode().getName(),
         storeArrayElementNode->getGlobalIndex(), storeArrayElementNode->getOpCode().getName());
      }
...
TR::TreeLowering::lowerStoreArrayElement(TR::Node *node, TR::TreeTop *tt)
   ...
   TR::TreeTop * nullCheckTT = tt->insertBefore(TR::TreeTop::create(comp, nullCheck));


   ///////////////////////////////////////
   // 3. Create the new ArrayStoreCHK, BNDCHK, NULLCHK
   TR::Node *anchoredElementIndexNode = anchoredElementIndexTT->getNode()->getFirstChild();
   TR::Node *anchoredArrayBaseAddressNode = anchoredArrayBaseAddressTT->getNode()->getFirstChild();
   ...
   //arrayStoreCHKTT = ttAfterHelperCall->insertBefore(TR::TreeTop::create(comp, TR::Node::create(node, TR::treetop, 1, arrayStoreCHKNode)));
   ...
   ///////////////////////////////////////
   // 4. Split at the helper call node including the nullchk on value into its own block helperCallBlock
   TR::Block *helperCallBlock = originalBlock->split(nullCheckTT, cfg);
   ...
   ///////////////////////////////////////
   // 5. Create the ificmpeq node that checks classFlags
   TR::SymbolReference *vftSymRef = comp->getSymRefTab()->findOrCreateVftSymbolRef();
   TR::SymbolReference *arrayCompSymRef = comp->getSymRefTab()->findOrCreateArrayComponentTypeSymbolRef();
   TR::SymbolReference *classFlagsSymRef = comp->getSymRefTab()->findOrCreateClassFlagsSymbolRef();
   ...
   ///////////////////////////////////////
   // 6. Split after the regular array element store
   ...
   ///////////////////////////////////////
   // 7. Set up the edges between the blocks
   ifNode->setBranchDestination(blockAfterHelperCall->getEntry());
