            ...
            for (cursor2.SetToFirstOne(); cursor2.Valid(); cursor2.SetToNextOne())
               ...
               for (Candidate *otherAllocNode = _candidates.getFirst(); otherAllocNode; otherAllocNode = otherAllocNode->getNext())
                 ...
                 if (!rhsIsHarmless &&
                    ...
                    if (/* (_valueNumberInfo->getValueNumber(defNode2) == _valueNumberInfo->getValueNumber(otherAllocNode->_node)) && */
                         (_useDefInfo->getTreeTop(defIndex2)->getEnclosingBlock() == otherAllocNode->_block) &&
