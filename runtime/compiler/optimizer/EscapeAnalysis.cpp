...
void TR_EscapeAnalysis::makeLocalObject(Candidate *candidate)
   ...
   TR::ByteCodeInfo bcInfo = allocationNode->getByteCodeInfo();
   TR::DebugCounter::prependDebugCounter(comp(), TR::DebugCounter::debugCounterName(comp(), "escapeAnalysis/contiguous-allocation/%s/(%s)/(%d,%d)", comp()->signature(), bcInfo.getCallerIndex() > -1 ? comp()->getInlinedResolvedMethod(bcInfo.getCallerIndex())->signature(trMemory()): comp()->signature(), bcInfo.getCallerIndex(), bcInfo.getByteCodeIndex(), candidate->_treeTop);
...
void TR_EscapeAnalysis::makeNonContiguousLocalAllocation(Candidate *candidate)
   ...
   if (candidate->objectIsReferenced())
      ...
      {
      TR::DebugCounter::prependDebugCounter(comp(), TR::DebugCounter::debugCounterName(comp(), "escapeAnalysis/noncontiguous-allocation/%s/(%s)/(%d,%d)", comp()->signature(), bcInfo.getCallerIndex() > -1 ? comp()->getInlinedResolvedMethod(bcInfo.getCallerIndex())->signature(trMemory()): comp()->signature(), bcInfo.getCallerIndex(), bcInfo.getByteCodeIndex(), candidate->_treeTop);
