         ...
         if (performTransformation(comp(), "%sStack allocating candidate [%p]\n",OPT_DETAILS, candidate->_node))
            ...
            if (candidate->escapesInColdBlocks())
               ...
               if (_initializedHeapifiedTemps == NULL)
                  ...
                  // on method entry.  Keep track of what is initialized on method entry to avoid redundant
                  // initializations that might occur in a large method that has many points at which
                  // heapification might occur.
