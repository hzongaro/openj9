...
#include "ValueTypeOpLowering.hpp"
...
int32_t
TR::ValueTypeOpLowering::perform()
   {
   ...
   for (TR::PreorderNodeIterator nodeIter(methodSymbol->getFirstTreeTop(), comp()); nodeIter != NULL ; ++nodeIter)
      ...
      if (node->getOpCode().isCall() && 
         ...
         if (!disableAcmpFastPath)
            {
            fastpathAcmpHelper(node, tt, comp()->getOption(TR_TraceCG));
            }
         }
...
const char *
TR::ValueTypeOpLowering::optDetailString() const throw()
   {
   return "O^O VALUE TYPE OP LOWERING: ";
   }
 ...
 */
void
TR::ValueTypeOpLowering::fastpathAcmpHelper(TR::Node *node, TR::TreeTop *tt, const bool trace)
   {
 ...
 */
void
TR::ValueTypeOpLowering::lowerArrayStoreCHK(TR::Node *node, TR::TreeTop *tt)
   {
   traceMsg(comp(), "INVOKED!!!\n");
